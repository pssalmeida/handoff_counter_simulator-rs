...
//println!("SERVER: {:?}, ", self.counter);
    ...
    fn client_periodic_handler(&mut self, global: &mut Global) -> Vec<Event<Self>> {
        ...
        if self.retiring && !self.counter.needs_to_handoff() {
//println!("RETIRED: {}", id);
            vec![Event { time: global.time, data: EventData::RetireNode(id) }]
        } else {
//if self.retiring { println!("CLIENT: {:?}, ", self.counter); }
...
fn statistics(state: &mut State<Node>) -> Vec<Event<Node>> {
    ...
    let t0s = state.t0ids.len();
    let t1s = state.t1ids.len();
    let slots_t1s = slots_sum as f64 / t1s as f64;
    let clients = state.nodes.len() - t0s - t1s;
    if state.global.time == 0 {
        println!("time\tclients\tactive\tids\tslots");
    } else {
    println!("{}\t{}\t{}\t{}\t{}",
             state.global.time, clients, active, ids_sum / t1s, slots_t1s);
    }
    vec![]
}

fn retire_node(state: &mut State<Node>) -> Vec<Event<Node>> {
    let n = state.t2ids.len();
    if n > 0 {
        let pos = state.global.rng.gen_range(0, n);
        let id = state.t2ids.remove(pos);
        let partitioned = {
            let node = &state.nodes[&id];
            state.global.rng.gen::<f64>() < node.partition_probability
        };
        if partitioned {
            vec![Event { time: state.global.time, data: EventData::RetireNode(id) }]
        } else {
            let node = &state.nodes[&id];
            vec![node.event(state.global.time, Retire)]
        }
    } else {
        vec![]
    }
...
fn run(args: &Args) {
    ...
    let partition_probability = args.partition_probability;
    let new_node = move |id: Id, tier: usize, peers: Vec<Id>| -> Node {
        Node {
            ...
            partition_probability: partition_probability as f64 / 100.0,
        }
    };
    ...
    sim.push_event(Event { time: 0, data: ed });
    let ed = EventData::Periodic(args.arrival_period, Box::new(retire_node));
    sim.push_event(Event { time: 0, data: ed });
...
fn main() {
    ...
    run(&Args {
        ...
        partition_probability: args[8].parse().unwrap(),
        end_time: args[9].parse().unwrap(),
        stat_interval: args[10].parse().unwrap(),
    });
}
